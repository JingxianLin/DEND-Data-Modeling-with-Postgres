...
def process_song_file(cur, filepath):
    ...
    for value in df.values:
        artist_id, artist_latitude, artist_location, artist_longitude, artist_name, duration, _, song_id, title, year = value
        # insert song record
...
def process_data(cur, conn, filepath, func):
    """
    Develop the ETL process for each table.
    
    Parameters:
        cur (psycopg2.cursor()): Cursor of the sparkifydb database
        conn (psycopg2.connect()): Connection to the sparkifydb database
        filepath (str): Filepath parent of the file to be analyzed
        func (function): Function used to process data
    """
